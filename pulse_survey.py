...
def soc(df):
    ...
    #array = indi_SoC.values.reshape(-1,1)
    scaler = preprocessing.MinMaxScaler(feature_range=(0, 100)) #normalize scores between 0 and 100
    indi_SoC_n =scaler.fit_transform(indi_SoC.values.reshape(-1,1)) # reshape array from 2d to 1d, and normalize
...
data = soc(pd.read_csv(sys.argv[1]))
