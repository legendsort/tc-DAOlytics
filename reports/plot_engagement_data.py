...
def plot_active_members(all_active, all_connected, all_periphery, all_core, \
	...
	act_type_colors = ['#a6cee3', '#1f78b4',  '#b2df8a', '#33a02c']  # for: new, unpaused, returned, continous respectively
	...
	mem_type_labels = ["Active", "Connected", "Vital"]
	...
	x_tick_labels = ["All", "New", "Unp.", "Ret.", "Cont.", "Conn.", "Vital"]
	...
	# add title
	all_core_ax.set_title('Vital history')
	...
	# add title
	all_ncore_ax.set_title('Non vital history')
	...
	if SAVE_PATH != None:
		act_mem_hist_fig.tight_layout()
		act_mem_hist_fig.savefig(SAVE_PATH + "_history_cut.png")
...
def plot_inactive_members(all_active, all_connected, all_paused, all_core, \
	...
	# set activity type colors
	#act_type_colors = ['#C85200', '#FF800E', '#FFBC79', '#CFCFCF']  # for: remainder, continuous, connected, vital respectively
	act_type_colors = ['#CFCFCF', '#FFBC79', '#FF800E', '#C85200']  # for: remainder, continuous, connected, vital respectively
	...
	# plot results	
	curr_ax.bar(mem_type_labels, layer4, bar_width, bottom=layer1+layer2+layer3, label='Vital', color=act_type_colors[3])
	...
	# specify x tick labels
	x_tick_labels = ["P. All", "P. Cont.", "P. Conn.", "P. Vital", "D. All", "D. Cont.", "D. Conn.", "D. Vital"]
	...
	# specify labels for plots
	act_type_labels = ["Rem.", "Cont.", "Conn.", "Vital."]
	...
	if SAVE_PATH != None:
		inact_mem_hist_fig.tight_layout()
		inact_mem_hist_fig.savefig(SAVE_PATH + "_history_cut.png")
...
def plot_unpaused_score(n_unpaused, n_new_disengaged, \
	date_tick_i, date_tick_labels, SHOW, SAVE_PATH=None):
	"""
	Plots active member graphs from engagement data
	
	Input:
	n_* - [int] : list with number of * accounts per analysis period
	date_tick_i - [int] : dictionary key values that correspond to dates
		that should be plotted as tick labels for the plots
	date_tick_labels - [str] : dates corresponding to the date_tick_i indices
	SHOW - bool : whether figure should be shown
	...
	Output:
	unp_mem_fig - fig : figure object with recent activity data
	Plots and/or saves figure based on SHOW and SAVE_PATH values
	"""
	
	# # # INITIAL PREPARATIONS # # #
	
	# add a 1 to n_unpaused and n_new_disengaged where both have value 0 to prevent divide by 0
	zero_mask = n_unpaused + n_new_disengaged == 0
	n_unpaused[zero_mask] = 1
	n_new_disengaged[zero_mask] = 1
	
	# compute ratio of unpaused vs newly disengaged
	plot_ratio = (np.array(n_unpaused)-np.array(n_new_disengaged)) / (np.array(n_unpaused)+np.array(n_new_disengaged))
	
	# initiate figure
	unp_mem_fig = plt.figure(figsize=(8,2))
	
	
	# # # PLOTTING FIGURE # # #
	
	# plot results
	plt.plot(range(len(plot_ratio)), plot_ratio, color='#33a02c', linewidth=2)
	plt.axhline(0, color="k", linewidth=2, dashes=(2,2))
	
	# add xtick labels
	plt.xticks(ticks = date_tick_i, labels = date_tick_labels)
	
	# add ytick labels
	plt.yticks(ticks = [-1,0,1], labels = ["All n.d.", "Even", "All u.p."])
	
	# adjust axes
	plt.xlim(date_tick_i[1],len(plot_ratio)-1)
	
	
	# show figure
	if SHOW:
		unp_mem_fig.tight_layout()
		plt.show()
		
	# save figure
	if SAVE_PATH != None:
		unp_mem_fig.tight_layout()
		unp_mem_fig.savefig(SAVE_PATH)
		print("Engagement unpausing members history figure saved in " + SAVE_PATH)
...
def plot_stacked_range(pl_layer1, pl_layer2, pl_layer3, pl_layer4, pl_lables, \
	...
	# adjust axes
	plt.xlim(date_tick_i[1],len(pl_layer1)-1)
...
def assess_overlap(ref_dict, comp_dicts, num_period, num_past, add_remainder, rand):
	...
	num_period - int: first num_period periods in ref_dict are used
	num_past - int : number of periods into the past that sets in 
